\pagestyle{empty}

% First column (wide)
\begin{minipage}[t][\pageheight]{0.65\textwidth-0.5\columnsep}
	\vspace{0pt}

	% Personal section
	\noindent\makebox[\columnwidth]{%
		\begin{minipage}{0.3\linewidth}
			\begin{tikzpicture}
				\clip (0,0) circle(0.5\textwidth);
				\node at (0,0) {\includegraphics[width=\textwidth]{media/johndoe.png}};
			\end{tikzpicture}
		\end{minipage}
		\begin{minipage}{0.6\linewidth}
			\raggedleft
			\textbf{\Huge John Doe}\\
			\vspace{0.1em}
			\textit{\large{Fullstack Developer}}
		\end{minipage}
	}

	\sectionsep

	% Contact information
	\noindent\makebox[\columnwidth]{%
		\begin{minipage}[c]{0.9\columnwidth}
			\centering
			\href{https://github.com/klepp0/resume}{\faLinkedinSquare \hspace{0.3em} john-doe} \hfill \href{https://github.com/klepp0/resume}{\faGithub \hspace{0.3em} johndoe} \hfill \faMapMarker \hspace{0.3em} Germany\\
			\href{tel:1234567890}{\faPhone \hspace{0.3em} (123) 456-7890}  \hfill \href{mailto:johndoe@example.com}{\faEnvelope \hspace{0.3em} johndoe@example.de}\\
		\end{minipage}
	}

	% Experience
	\sectiontitle{Experience}

	\textbf{\large Software Developer} \hfill \href{https://github.com/klepp0/resume}{Tech Company Inc.}\\
	\textit{\color{darkgray} June 2019 -- Present}
	\vspace{-0.8em}
	\begin{itemize}
		\item Developed and maintained web applications using React and Node.js. Saved the company from certain doom with my wizard-like coding skills.
		\item Collaborated with cross-functional teams to define and achieve project goals, while mastering the art of surviving endless meetings.
		\item Implemented RESTful APIs and integrated third-party services like a boss.
		\item Improved application performance, reducing load time by 30\%. My code runs faster than a cheetah on Red Bull.
	\end{itemize}
	\vspace{-0.8em}
	{\color{darkgray}\texttt{node.js}\slashsep\texttt{RESTful-API}}

	\subsectionsep

	\textbf{\large Intern} \hfill \href{https://github.com/klepp0/resume}{Another Tech Company}\\
	\textit{\color{darkgray} June 2018 -- August 2018}
	\vspace{-0.8em}
	\begin{itemize}
		\item Assisted in the development of an internal tool for data analysis. Basically, I turned coffee into code.
		\item Participated in code reviews and contributed to the improvement of code quality. I'm the secret sauce behind those spotless codebases.
		\item Wrote unit tests to ensure code reliability. My tests are more reliable than your morning coffee.
	\end{itemize}
	\vspace{-0.8em}
	{\color{darkgray}\texttt{coffee machine}\slashsep\texttt{git}}

	% Project section
	\sectiontitle{Projects}

	\textbf{\large Project One}
	\vspace{-0.8em}
	\begin{itemize}
		\item Developed a full-stack web application for task management using React and Node.js. Yes, another to-do list app, but this one is special.
		\item Implemented user authentication and authorization with JWT. Keeping the bad guys out, one token at a time.
	\end{itemize}
	\vspace{-0.8em}
	{\color{darkgray}\texttt{node.js}\slashsep\texttt{React}}

	\subsectionsep

	\textbf{\large Project Two}
	\vspace{-0.8em}
	\begin{itemize}
		\item Created a machine learning model to predict stock prices using Python. Spoiler: it didn’t make me a millionaire.
		\item Visualized data trends and model performance with Matplotlib and Seaborn. Because pretty graphs make everything better.
	\end{itemize}
	\vspace{-0.8em}
	{\color{darkgray}\texttt{Python}}

	\vfill\null
\end{minipage}
\hfill
% Second column (slim)
\begin{minipage}[t][\pageheight]{0.35\textwidth-0.5\columnsep}
	\vspace{0pt}

	% Profile description
	\sectiontitle{Profile}
	I'm a passionate software developer who loves crafting products that people love. I'm a huge lover of Open Source and also volunteer in many communities. I care about making the customers happy, testing, accessibility, teamwork, and code quality. When I'm not coding, I'm probably thinking about coding.

	% Skills section
	\sectiontitle{Skills}
	\vspace{-0.8em}
	\begin{itemize}
		\item \textbf{Programming Languages:} \\
		      \texttt{Python, Java, C++}
		\item \textbf{Web Development:} \\
		      \texttt{HTML, CSS, JavaScript, React}
		\item \textbf{Databases:} \\
		      \texttt{MySQL, PostgreSQL, MongoDB}
		\item \textbf{Tools:} \\
		      \texttt{Git, Docker, Jenkins}
	\end{itemize}
	\vspace{-0.8em}

	% Language section
	\sectiontitle{Languages}
	\noindent\makebox[\columnwidth]{%
		\begin{minipage}[t]{0.9\columnwidth}
			\textbf{German}  \hfill\rating{5} \\
			\textbf{English} \hfill\rating{4} \\
			\textbf{Spanish} \hfill\rating{3}
		\end{minipage}
	}

	% Open Source section
	\sectiontitle{Open Source}
	I have contributed to projects such as Mock Service Worker (used by thousands) and Testing Library (used by millions). I also volunteer in communities like KCD and Remix. When I say I’m into Open Source, I mean it – I dream in pull requests.

	\sectiontitle{Interests}
	Qudditch, Speed Typing with One Finger, Inventing New Sandwiches

	\vfill

	\begin{flushright}
		\textit{\today}
	\end{flushright}
\end{minipage}

